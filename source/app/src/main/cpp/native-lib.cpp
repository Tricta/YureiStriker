...
static void *allocTrampolineSpace() {
    unsigned char *buffer = (unsigned char*)mmap(NULL, TRAMPOLINE_SPACE_SIZE, PROT_READ | PROT_WRITE | PROT_EXEC, MAP_ANONYMOUS | MAP_PRIVATE, -1, 0);
    ...
    for(int i = 0; i< sizeof(methods) / sizeof(JNINativeMethod); i++){
        ...
        if(methodId != nullptr){
            if(env->RegisterNatives(targetClass, &methods[i], 1) != JNI_OK){
                __android_log_print(ANDROID_LOG_INFO, "YureiStriker", "[YureiStriker]: Error on register native methods");
            }
        }
    }
}
...
JNIEXPORT jint JNICALL JNI_OnLoad(JavaVM *vm, void *reserved) {
    ...
    jclass helldroidClass = env->FindClass("com/example/yureistriker/MainActivity");
    if(!helldroidClass){
        return JNI_ERR;
    }
